...
public class CakeThief {
    ...
    public static long maxDuffelBagValue(CakeType[] cakeTypes, int weightCapacity) {
        ...
        long[] maxValueAtCapacity = new long[weightCapacity + 1];
        ...
        for(int currentCapacity=0; currentCapacity<= weightCapacity; currentCapacity++) {
            ...
            for(CakeType cakeType: cakeTypes) {
                ...
                if(cakeType.weight == 0 && cakeType.value != 0) {
                    ...
                    throw new RuntimeException("Enccountered Infinite case");
                }

                if(cakeType.weight > currentCapacity) continue;

                long valueUsingCakeType = cakeType.value + maxValueAtCapacity[currentCapacity - cakeType.weight];

                currentMaxValue = Math.max(valueUsingCakeType, currentMaxValue);
            }

            maxValueAtCapacity[currentCapacity] = currentMaxValue;
        }

        return maxValueAtCapacity[maxValueAtCapacity.length - 1];
    }
